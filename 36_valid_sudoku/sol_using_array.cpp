...
class Solution {
   public:
    bool isValidSudoku(vector<vector<char>>& board) {
        const int cnt = 9;
        bool row_map[cnt][cnt] = {false};  // 0-8
        bool col_map[cnt][cnt] = {false};  // 0-8
        bool box_map[cnt][cnt] = {false};  // 0-8
        ...
        for (int i = 0; i < 9; ++i) {
            for (int j = 0; j < 9; j++) {
                ...
                int element = board[i][j] - '0' - 1;  //zero index array so 9 needs to be 8
